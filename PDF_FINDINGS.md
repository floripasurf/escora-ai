# Supplier Training PDF — Rule Extraction Catalog

> Source: `Treinamento_Tecnico_Escoramento-20-11-2020.pdf`, Grupo Supplier, Nov 2020 (123 pages).
>
...
- pp.31–32 show a real Supplier project drawing: layout-out de torres e vigas metálicas with VM130-410, VM130-310, VM80-255 etc. labels — direct evidence of how the BOM annotates the layout. `[reference for output format]`
...
- **Mecaner system** (Supplier proprietary) for nervuradas:
  ...
  - **Cubeta dimensioning rule (p.44):** Cubeta width must be (nominal − 7.5 cm) on the régua side. Example: a "80 cm" cubeta must be 80 × 72.5 cm (réguas Supplier are 7.5 cm wide).
...
- **p.60 (legal):** All reescoramento projects produced by Supplier's Engenharia de Aplicação are sent **as suggestion only**. Responsibility for use stays with the obra's engineer or estrutural calculista. **This is the critical legal framing for our auditable-draft model.**
...
### Rule 2 (p.98) `[new]`
Vertical shoring must be **aprumado** (plumb). Out-of-plumb significantly reduces equipment capacity. The Supplier diagrams show that even small deviations alter resistance "de forma significativa."
...
### Rule 8 (p.104) `[new]`
**Não alterar o projeto sem comunicação prévia à Supplier.**

### Rule 9 (p.104) `[new]`
For montagem problems, consult Supplier for joint review of the layout.

### Rule 10 (p.104) `[new]`
**Travamento e amarração das fôrmas** is the client's responsibility (not Supplier's scope).
...
- p.123 — Engenharia de Aplicação contact (the Supplier team that produces final projects)
