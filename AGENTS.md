...
>
> **Document version:** 2026-05-04 (v3) — comprehensive update after visual inspection of all 123 pages of Supplier training PDF.
...
- **Output:** shoring layout DXF with Supplier symbology, BOM CSV, Memória de Cálculo PDF, IFC for BIM integration.
- **Equipment partner:** Grupo Supplier.
- **Calibration baseline:** 12 executive Supplier projects, calibrated 2026-04-07.
- **Legal framing:** the system produces an **auditable draft**, sent to the engineer as a **suggestion** (per Supplier training p.60). Final responsibility stays with the obra's engineer or estrutural calculista. Every output PDF must include the unfilled ART block at the end.

---

## Domain Authority (priority order)

When sources conflict, higher-priority sources win. NBR norms are the legal foundation; Supplier materials provide operational specifics; engineer Q&A reflects calibrated practice.
...
8. **Supplier internal training** — *Treinamento Técnico Escoramento*, Nov 2020, 123 pp. Visually inspected page-by-page; full extraction in `PDF_FINDINGS.md`.
9. **Supplier engineer Q&A** — Calibration session 2026-04-07, n=12 projects.
...
|---|---|---|
| Sobrecarga de trabalho (área distribuída, concretagem) | **2.0 kN/m² = 204 kgf/m²** | NBR 15696 §4.2 + Supplier p.26 |
| Plataforma de trabalho (carga local em passarelas) | 1.5 kN/m² = 153 kgf/m² | NBR 15696 §4.2 + Supplier p.27 |
| Carga estática total mínima | 4.0 kN/m² = 408 kgf/m² | NBR 15696 §4.2 + Supplier p.26 |
| Vento mínimo (escoramento >10 m ou aberto) | 0.6 kN/m² = 61.2 kgf/m² | NBR 15696 §4.2 + NBR 6123 + Supplier p.26 |
| Vento adicional lateral | +5% V | Supplier p.27 (load diagram) |
| γc (concreto) | 25 kN/m³ = 2550 kgf/m³ | NBR 6120 + Supplier p.26 |
| γf (majoração de cargas) | 1.4 | NBR 15696 |
| Acréscimo em apoio central de viga contínua 3+ apoios | +25% (10/8 q·L) | Supplier p.109 (regra 14) |
| Reação em extremidades de viga contínua | 3/8 q·L | Supplier p.109 (regra 14) |

### Two simultaneous load systems — critical clarification

The Supplier training **page 27 load diagram** (image-only) shows both loads applied simultaneously to different parts of the structure:
...
## Minimum Substrate Stress (NBR 15696 reference block, Supplier p.25)
...
| Distância da borda da laje (geral) | 0.15 m | Supplier practice (validated 2026-04-07) |
| Espaçamento mínimo entre escoras adjacentes | 0.30 m | Supplier practice (validated 2026-04-07) |
| Distância de alvenaria estrutural (laje encostada) | ≤ 5 cm | Supplier p.107 (regra 12) |

### Edge offset of barrotes (Supplier p.105–106, regra 11) — corrected
...
### Cotas reference rule (Supplier p.23)
...
## Spacing — Slab Thickness × Plywood Combination Table (Supplier p.89)
...
### Plywood Seam Alignment Constraint (Supplier p.114–115, regra 17)
...
## Compensado (Plywood) Properties — Supplier p.78
...
## Slab Typology — Routing per Type (Supplier pages 28–49)
...
| **Nervurada** | Cubeta grid pattern (square or rectangular cubetas) | **Mecaner system preferred** when polipropileno cubetas. Réguas Supplier are 75 mm or 30 mm wide. Cubeta width must be (nominal − 7.5 cm) on régua side |

### Vão máximo das vigotas

For pré-moldadas: comum até 5 m entre apoios (Supplier p.33). The vão máximo is set by the vigota fabricante; system must accept this as input parameter, not compute it.

---

## Decision Chain v3 (engineer-validated, expanded)

The system selects support type by walking these questions in order. **The first matching question wins.** Do not reorder. Calibrated against 12 Supplier projects (2026-04-07).
...
**Note on Q1 vs. Q9 from p.9:** for level supports with vão entre vigas ≤ 4 m AND pé direito ≤ 3.50 m, telescópicas alone are the recommended solution (Supplier comfort zone). Between 3.50 and 4.50 m, telescópicas are still possible but require explicit verification of derated capacity. **Open question:** confirm with engineers whether to add a separate Q1.5 for the 3.50–4.50 m intermediate band.

---

## Tower Capacity Curve — Supplier p.86
...
† PDF chart shows "14 = 1500" but this breaks the monotonic curve and is almost certainly a typo for 1590. **Confirm with Supplier before encoding.** Until confirmed, the system flags any 14-module placement for engineer review.

---

## Telescópica Capacity Curves — Supplier p.11
...
## Equipment Catalog (Supplier — comprehensive)

**Single source of truth: `catalog/equipment.yaml`.**

### VM Properties (Supplier p.69)
...
### Painéis de torre (Supplier p.13–16)
...
### Tower height calculation rule (Supplier p.89, Example 2)
...
### Pillar bracing (Supplier pages 62–64)
...
| Face | Estratégia | Materiais (per Supplier example) |
|---|---|---|
...
### Lateral bracing of beams (Supplier pages 65–66)
...
### Beam bottom bracing (Supplier p.67)
...
### Hard rules (Supplier p.55, p.59)

| Rule | Value | Source |
|---|---|---|
| Vão livre máximo no reescoramento | **2.0 m** | Supplier p.59 |
| Reescoramento 50% (load fraction) | 50% × carga_escoramento | Supplier p.59 |
| Reescoramento 25% (load fraction) | 25% × carga_escoramento | Supplier p.59 |
| Reescoramento remanescente (during upper concretagem) | full load + upper levels' loads | Supplier p.59 |
| Sem desforma + sem liberação | 200% carga (bad practice — reject) | Supplier p.55 |
...
All reescoramento projects from Supplier's Engenharia de Aplicação are **suggestions only**. Responsibility for use stays with the obra's engineer or estrutural calculista.
...
## Mandatory Output Header (per Supplier p.25)
...
These are non-negotiable engineering practices from Supplier pages 97–115.
...
| OP-008 | Não alterar projeto sem comunicação à Supplier | p.104 (rule 8) |
...
These items still require confirmation from Supplier engineers.
...
5. **Special-case routing.** Supplier pp.116–121 describe special cases (Alu-14, grandes alturas, escoramento aéreo, H20 rampas, projetos especiais). Should the system route to "fora do automático" with engineer notification when these characteristics are detected, or attempt automated handling?
...
class Source(BaseModel):
    ...
    ref: str          # "NBR 15696:2009 §4.2.1" or "Supplier p.86"
    calibration: str | None = None  # "Supplier 2026-04-07 (n=12)"
```

Cite at this granularity:
- "Supplier p.86" — for the tower derating curve
- "Supplier p.89" — for the plywood span table
- "Supplier p.111" — for external beam rules
- "Supplier p.112" — for internal beam rules
- "Supplier p.59" — for reescoramento vão limit
- "Supplier 2026-04-07 (n=12)" — for engineer-validated heuristics (35%, 18%, 15%, 12-16 kg/m³)
...
encoding rules from the Supplier training material.
