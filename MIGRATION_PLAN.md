...
2. **Do not invent norm references or numeric values.** Every cited NBR section, every numeric threshold, every spacing rule must come from `AGENTS.md`, the Supplier training extracts, or the engineer Q&A. If something seems necessary but isn't documented, flag it as a `# TODO(engineer-confirmation)` and stop.
...
4. **Do not modify the regression suite to make it pass.** The 12 Supplier projects are ground truth. If your change breaks them, your change is wrong.
...
Escora.AI generates shoring (escoramento) layouts for Brazilian civil construction. Inputs: structural plan DXFs (typically from TQS, AutoCAD, or Eberick). Outputs: shoring layout DXF (Supplier symbology), BOM CSV, Memória de Cálculo PDF, IFC for BIM integration. Calibrated against 12 executive Supplier projects (April 2026). Equipment partner: Grupo Supplier.
...
**Objective:** Build a complete inventory of current defects and tacit rules. This phase produces no code changes. It produces a list of bugs you currently have, a list of rules you currently lack, and a list of patterns from real Supplier projects that your system fails to reproduce.
...
> 1. Discovers all reference projects under `tests/calibration/` (or wherever the 12 Supplier DXFs live — confirm location with the operator before proceeding).
...
>
> One specific item: the current code uses `sobrecarga de trabalho = 1.50 kN/m²` but NBR 15696 §4.2 specifies a minimum of 2.0 kN/m². Mark this in `AGENTS.md` under "Open Engineering Questions" with the exact quote from the Supplier training PDF (page where it specifies the 2.0 minimum). Do not change the constant in code yet.
>
...
> Create `tests/regression/conftest.py` with a parameterized fixture `calibration_project` that yields each of the 12 Supplier projects. Each yielded value is a tuple `(project_id, input_dxf_path, signed_bom_path)`.
>
...
**Objective:** Encode the first ~15 rules that can be extracted unambiguously from `AGENTS.md`, the engineer Q&A, and the Supplier training. These prove the architecture works end-to-end and establish the pattern for all subsequent rules.
...
**Objective:** Use the 12 Supplier project DXFs as ground truth. Extract empirical patterns. Anywhere your system's output systematically differs from real Supplier work, that's a missing rule.

## Task 3.1 — Pattern extractor for Supplier reference DXFs

**Prompt for Claude Code:**

> Create `scripts/empirical/extract_patterns.py`. It reads each Supplier reference DXF (the *output* DXFs from real signed projects, not the input structural plans) and computes per project:
>
...
>
> Output: `diagnostics/supplier_patterns.yaml` with one section per project plus an `aggregate` section summarizing across all 12.
>
> Use ezdxf + shapely. Pure read; no modifications to anything.
>
> **Verification gate:** `supplier_patterns.yaml` exists with all 12 projects + aggregate. Operator reviews aggregate values and confirms they match expectations (e.g., median tower-to-telescópica ratio is in the expected range).

## Task 3.2 — Compare system output against patterns

**Prompt for Claude Code:**

> Create `scripts/empirical/compare_outputs.py`. For each calibration project, compare the system's generated output against the Supplier reference output along the same dimensions extracted in Task 3.1.
>
...
> - The pattern observed in Supplier projects (with quantitative summary)
...
- [ ] `supplier_patterns.yaml` extracted from all 12 reference DXFs
- [ ] `output_deltas.yaml` quantifies system vs Supplier differences
...
**Objective:** Encode the next ~30 rules. Mix of: high-confidence empirical candidates from Phase 3, rules extracted from the Supplier training PDF, and rules from the engineer Q&A that didn't make Phase 2.

## Task 4.1 — Extract rules from Supplier training PDF

**Prompt for Claude Code:**

> Read the Supplier training material text (already extracted to `docs/supplier_training_extract.md` from the original PDF). Walk through it section by section. For every operational rule you can identify, add an entry to `rules/from_supplier_training.md` with:
...
>
> **Verification gate:** `rules/from_supplier_training.md` has at least 30 candidate rules with citations. Operator reviews and approves implementation order.

## Task 4.2 through 4.N — Implement approved rules

For each rule the operator approves from `rules/from_supplier_training.md` and `rules/candidates.md`, repeat the Task 2.1 pattern: unit tests + regression delta + commit per rule.
...
Output: shoring layout DXF with Supplier symbology, BOM CSV, Memória 
...
8. Supplier internal training (Treinamento Técnico Escoramento, 
   Nov 2020) — operational rules and worked examples.
9. Supplier engineer Q&A (calibration date 2026-04-07, n=12 executive 
   projects).
...
| Acréscimo de carga em apoio central contínuo | +25% | Supplier training §7 (hiperestaticidade) |
...
- The current code uses `sobrecarga de trabalho = 1.50 kN/m²` but 
  ...
  confirm with Supplier engineers whether this is intentional 
...
| Distância da borda da laje | 0.15 m | Practice (Supplier-validated) |
| Espaçamento entre escoras | 0.30 m | Practice (Supplier-validated) |
| Distância de alvenaria estrutural | ≤ 5 cm | Supplier training §8 (regra 12) |
| Cantilever (balanço): tip → support | 20–40 cm | Supplier training §8 (regra 11) |
...
5. Viga com (laje ≥ 15 cm OR vão > 6 m) → MISTO 35% torres em 
   extremidades e interseções (Supplier calibration 2026-04-07)
...
## Validated Calibration Envelope (Supplier, 2026-04-07, n=12)
...
class Source(BaseModel):
    ...
    ref: str = Field(
        ..., 
        description="Citation: 'NBR 15696:2009 §4.2.1' or 'Supplier "
                    "training p.42' or 'Engineer Q&A #5'"
    )
    calibration: Optional[str] = Field(
        None, 
        description="When applicable: 'Supplier 2026-04-07 (n=12)'"
    )
...
| GEOM-002 | GEOM | Distância mínima de 0.15 m da borda da laje | Supplier practice + AGENTS.md | error |
| GEOM-003 | GEOM | Espaçamento mínimo de 0.30 m entre escoras adjacentes | Supplier practice + AGENTS.md | error |
...
| GEOM-005 | GEOM | Distância máxima de barrotes para alvenaria estrutural ≤ 5 cm | Supplier training §8 regra 12 | warning |
| GEOM-006 | GEOM | Extremidade de barrote em balanço: distância da última escora entre 20 e 40 cm | Supplier training §8 regra 11 | warning |
...
| STRUCT-002 | STRUCT | Cantilever (balanço): suporte presente a ≤ 40 cm da extremidade | Supplier training §8 regra 11 | error |
...
| LOAD-003 | LOAD | Em vigas contínuas com 3+ apoios: acréscimo de 25% na reação central | Supplier training §7 (hiperestaticidade) | error |
...
| EQUIP-002 | EQUIP | Equipamento selecionado existe no catálogo Supplier | catalog/equipment.yaml | error |
| EQUIP-003 | EQUIP | Utilização de torres entre 55% e 85% da capacidade | Engineer Q&A #10 | warning |
| ENV-001 | ENV | kg/m³ total no envelope [12, 16] | Engineer Q&A #8, Supplier 2026-04-07 (n=12) | warning |
...
2. Do not invent norm references or numeric values. Every cited NBR 
   ...
   from AGENTS.md, the Supplier training extracts, or the engineer 
...
4. Do not modify the regression suite (tests/regression/) without 
   explicit operator approval. The 12 Supplier projects are ground 
   truth.
