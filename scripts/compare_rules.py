"""Compare Supplier decision rules against current Escora.AI engine logic.

Reads decision_rules from supplier_rules_extracted.json and evaluates
whether our engine's decision logic matches what Supplier engineers do.
...
- Are we missing rules that Supplier applies implicitly?
...
def load_analysis(path="data/analysis/supplier_rules_extracted.json") -> dict:
...
def generate_report(data: dict) -> str:
    ...
    lines.append("# Comparação: Critérios de Decisão Supplier vs Escora.AI")
    lines.append("")
    ...
    if r:
        ...
        lines.append("")
        lines.append(f"Supplier usa **{r.get('tower_fraction', 0):.0%}** torres em vigas "
        ...
        lines.append(f"- **Problema**: fração fixa não reflete critério estrutural — "
                    f"Supplier coloca torres ONDE a carga exige, não em X% das posições")
        lines.append("")
    ...
    if r:
        ...
        lines.append("")
        lines.append(f"Supplier usa **{r.get('tower_fraction', 0):.0%}** torres em lajes "
                     f"({r.get('n_towers', 0)} torres, {r.get('n_shores', 0)} escoras)")
        lines.append("")
    ...
    lines.append("e maior em lajes finas — como Supplier faz.")
    lines.append("")
    lines.append("### 5c. Densificação próxima a pilares")
    if sp and any(data.get("densifies_near_pillar") for data in sp.values()):
        lines.append("DETECTADO: Supplier densifica o espaçamento perto dos pilares.")
...
def main():
    ...
    path = "data/analysis/supplier_rules_extracted.json"
    if not Path(path).exists():
        print(f"ERROR: {path} not found. Run analyze_supplier_rules.py first.")
        sys.exit(1)
