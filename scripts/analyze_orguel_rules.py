"""Reverse-engineer shoring DECISION CRITERIA from Supplier PE DXF files.
...
derives the rules Supplier engineers apply:
...
def extract_decision_rules(dataset: List[dict]) -> dict:
    ...
    # =====================================================================
    # RULE 1: Tower vs Shore on BEAMS — when does Supplier choose tower?
    # =====================================================================
    if viga_entities:
...
def analyze_file(filepath: str) -> Tuple[Optional[dict], List[dict]]:
    """Analyze a single Supplier DXF. Returns (file_summary, entity_rows)."""
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Extract Supplier shoring decision rules")
    parser.add_argument("--files", type=str, default=None)
    parser.add_argument("--output", type=str, default="data/analysis/supplier_rules_extracted.json")
    ...
    if args.files:
        ...
        supplier_dir = project_root / "input" / "supplier"
        files = sorted(str(f) for f in supplier_dir.glob("*.dxf"))
    ...
    print("=" * 70)
    print("  ESCORA.AI — Supplier Decision Rules Extraction")
    print("=" * 70)
